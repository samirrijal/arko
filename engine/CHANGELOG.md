...
`Project` documents from an Arko `Study` + `Computed` pair. Closes
the Phase-1 execution-guide "EPDX read/write works" bullet via the
living successor: EPDX was archived by its upstream authors
(ocni-dtu) on 2024-08-22 and superseded by LCAx (same maintainers,
same Apache-2.0 license, actively developed). Full rationale in
`DECISIONS.md` `D-0018`; staged plan places the Phase-2 ILCD+EPD
writer (the actual Environdec submission format) before the Phase-2
EPD-renderer milestone.

**Scope (V1):**

- **Writer only.** Reading LCAx documents into Arko studies is a
  separate V2 concern.
- **Full Project wrapper.** LCAx's root type is `Project`;
  standalone Product/EPD documents are not valid. V1 emits a
  synthetic single-Assembly / single-Product `Project` wrapping one
  EPD — the minimum-viable nesting required by the schema.
- **All impact values at `LifeCycleModule::A1A3`.** Arko doesn't
  model life-cycle stages; A1A3 (cradle-to-gate) is the truthful
  default for the unit-process LCAs Arko fixtures represent. Stage
  decomposition is Phase 2-3.
- **`standard` enum mapping.** Of the four V1 method presets, only
  `ef-3.1` maps cleanly to `Standard::EN15804A2`; the other three
  (`ipcc-ar6-gwp100`, `ipcc-ar5-gwp100`, `cml-ia-baseline`) emit as
  `Standard::UNKNOWN` because the schema enum has no matching
  variant. The method identifier is always preserved in
  `EPD.comment` as `"Generated by Arko using method <id>@<version>"`
  so downstream consumers can recover the methodology even when
  `standard == UNKNOWN`. See refinement #2 in `D-0018`.

**API:** Single public entry point
`write_lcax_project(study, computed, metadata) -> Result<Project,
WriteError>`. `EpdDocumentMetadata` carries the ~10 EPD-domain
fields LCAx requires but Arko's `Study` doesn't model
(`reference_service_life`, `published_date`, `valid_until`,
`subtype`, `location`, `project_phase`, `software_info`, etc.). The
type is deliberately named `EpdDocumentMetadata`, not
`WriterConfig` — the same shape will feed the Phase-2 ILCD+EPD
writer unchanged. Construct via
`EpdDocumentMetadata::with_product_name(..)` for the typical case;
every field has a sensible default so a one-liner produces a
schema-conformant document.

**Dependencies:** `lcax_models = "3.4"` (data types),
`lcax_core = "3.4"` (`Country` + date serde helpers). Both are
Apache-2.0 despite the crates.io "non-standard" classifier — that
flag is a packaging convention (Cargo.toml uses `license-file`
instead of `license = "Apache-2.0"`); the LICENSE file is plain
Apache-2.0 with no carve-outs. No LCI dataset attribution required:
LCAx is a format + library, not a dataset.

**Tests:** 9 total — 7 unit (standard/subtype/impact-id mappings,
slug hygiene, metadata defaults) + 2 smoke
(`tests/minimal_writer_smoke.rs`) exercising the spec §16 two-process
study through the pipeline → writer → `serde_json` → `Project`
round-trip, proving the emitted JSON is schema-shaped (any field
divergence surfaces as a round-trip deserialisation error).
