...
fn mass_chain() {
    ...
    assert_relative_eq!(
        convert_str(1.0, "kg", "g").unwrap(),
        1000.0,
        epsilon = 1e-9
    );
    assert_relative_eq!(
        convert_str(1.0, "t", "g").unwrap(),
        1e6,
        epsilon = 1e-3
    );
    assert_relative_eq!(
        convert_str(1.0, "mg", "g").unwrap(),
        1e-3,
        epsilon = 1e-12
    );
}
...
fn length_chain() {
    assert_relative_eq!(
        convert_str(1.0, "km", "m").unwrap(),
        1000.0,
        epsilon = 1e-9
    );
    assert_relative_eq!(
        convert_str(1.0, "cm", "m").unwrap(),
        1e-2,
        epsilon = 1e-15
    );
    assert_relative_eq!(
        convert_str(1.0, "mm", "m").unwrap(),
        1e-3,
        epsilon = 1e-15
    );
    assert_relative_eq!(
        convert_str(1.0, "um", "mm").unwrap(),
        1e-3,
        epsilon = 1e-15
    );
}
...
fn volume_chain() {
    assert_relative_eq!(
        convert_str(1.0, "L", "m3").unwrap(),
        1e-3,
        epsilon = 1e-15
    );
    assert_relative_eq!(
        convert_str(1.0, "mL", "L").unwrap(),
        1e-3,
        epsilon = 1e-15
    );
    assert_relative_eq!(
        convert_str(1.0, "m3", "L").unwrap(),
        1000.0,
        epsilon = 1e-9
    );
}
...
fn energy_chain() {
    ...
    assert_relative_eq!(
        convert_str(1.0, "kWh", "J").unwrap(),
        3.6e6,
        epsilon = 1e-3
    );
    assert_relative_eq!(
        convert_str(1.0, "MJ", "J").unwrap(),
        1e6,
        epsilon = 1e-3
    );
    assert_relative_eq!(
...
fn time_chain() {
    ...
    assert_relative_eq!(
        convert_str(1.0, "d", "h").unwrap(),
        24.0,
        epsilon = 1e-12
    );
    // 1 yr = 365.25 days.
    assert_relative_eq!(
        convert_str(1.0, "yr", "d").unwrap(),
        365.25,
        epsilon = 1e-9
    );
}
